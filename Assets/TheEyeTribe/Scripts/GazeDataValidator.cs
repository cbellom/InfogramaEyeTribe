...
namespace CompleteProject
{
    ...
    class GazeDataValidator
    {
		...
		protected bool _IsGazeCoordsNull;
		protected bool _IsGazeRightNull;
		protected bool _IsGazeLeftNull;
        ...
        public virtual void Update(GazeData frame)
        {
            ...
            lock (_GazeFrameCache)
            {
                for (int i = _GazeFrameCache.Count; --i >= 0; )
                {
					...
					SetEyeStatesOfDataCaptured (right, left, gazeCoords, userPos, gd);
		...
		void SetEyeStatesOfDataCaptured (Eye right, Eye left, Point2D gazeCoords, Point2D userPos, GazeData gd)
		{
			if (null == userPos && right == null && gd.RightEye.PupilCenterCoordinates.Equals (Point2D.zero)) {
				_IsGazeRightNull = true;
			}
			else {
				_IsGazeRightNull = false;
			}
			if (null == userPos && left == null && gd.LeftEye.PupilCenterCoordinates.Equals (Point2D.zero)) {
				_IsGazeLeftNull = true;
			}
			else {
				_IsGazeLeftNull = false;
			}
			if (null != gazeCoords)
				_IsGazeCoordsNull = false;
			else
				_IsGazeCoordsNull = true;
		}
		...
		public bool IsGazeCoordsNull() {
			return _IsGazeCoordsNull;
		}

		public bool IsGazeRightNull() {
			return _IsGazeRightNull;
		}

		public bool IsGazeLeftNull() {
			return _IsGazeLeftNull;
		}
